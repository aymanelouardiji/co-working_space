...
import { bookingService, deskService, amenityService, workspaceService } from '../services/api';
...
const WorkspaceDetail = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const [workspaceRes, deskRes, amenityRes] = await Promise.all([
          workspaceService.getById(id),
          deskService.getByWorkspace(id),
          amenityService.getAvailable(),
        ]);
  ...
  const handleBooking = async (e) => {
    ...
    try {
      ...
      setError(err.response?.data?.message || 'Booking failed. Check for overlaps.');
    }
  };
  ...
  if (loading) return <div>Loading workspace details...</div>;
  ...
  return (
    <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr', gap: '2rem' }}>
        ...
        <div className="glass-card" style={{ background: 'white', padding: '1.5rem', marginBottom: '2rem' }}>
            ...
            {desks.map((desk) => (
              <div
                ...
                style={{
                  ...
                  backgroundColor:
                    desk.availabilityStatus === 'AVAILABLE'
          ...
          <div style={{ marginTop: '1.5rem', display: 'flex', gap: '1rem', fontSize: '0.8rem' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '0.4rem' }}>
              <div style={{ width: '12px', height: '12px', background: 'white', border: '1px solid var(--border)' }} />
              Available
            </div>
            <div style={{ display: 'flex', alignItems: 'center', gap: '0.4rem' }}>
              <div style={{ width: '12px', height: '12px', background: 'var(--secondary)' }} />
              Selected
            </div>
            <div style={{ display: 'flex', alignItems: 'center', gap: '0.4rem' }}>
              <div style={{ width: '12px', height: '12px', background: '#f1f5f9' }} />
              Occupied
            </div>
          </div>
        </div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(140px, 1fr))', gap: '1rem' }}>
            {amenities.map((amenity) => (
              ...
              >
                <div
                  style={{
                    color: selectedAmenities.includes(amenity.id) ? 'var(--secondary)' : 'var(--text-muted)',
                    marginBottom: '0.5rem',
                  }}
                >
      ...
      <div
        ...
        style={{ background: 'white', padding: '2rem', height: 'fit-content', position: 'sticky', top: '2rem' }}
      >
        ...
        <form onSubmit={handleBooking}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem', marginBottom: '1.5rem' }}>
            <div>
              ...
              <input
                type="time"
                required
                style={{ width: '100%', padding: '0.6rem', border: '1px solid var(--border)', borderRadius: '0.4rem' }}
                value={startTime}
          ...
          <div style={{ borderTop: '1px solid var(--border)', paddingTop: '1.5rem', marginBottom: '1.5rem' }}>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between' }}>
              ...
              <span style={{ fontWeight: 600 }}>{workspace.workspaceName}</span>
            </div>
          </div>
