    ...
    const fetchDashboardData = async () => {
      try {
        ...
        const counts = bookings.reduce((acc, booking) => {
          acc[booking.bookingStatus.toLowerCase()] = (acc[booking.bookingStatus.toLowerCase()] || 0) + 1;
          return acc;
        }, {});
    ...
    <div>
      <div className="stat-grid">
        <div className="glass-card" style={{ padding: '1.5rem', background: 'white' }}>
          ...
          <div style={{ fontSize: '0.8rem', color: 'var(--success)', marginTop: '0.5rem' }}>Updated this week</div>
        </div>
        <div className="glass-card" style={{ padding: '1.5rem', background: 'white' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1rem' }}>
              ...
              {recentBookings.map((booking) => (
                <tr key={booking.id} style={{ borderBottom: '1px solid #f1f5f9' }}>
                  <td style={{ padding: '1rem' }}>
                    <div style={{ fontWeight: 600 }}>{booking.desk.workspace.workspaceName}</div>
                    <div style={{ fontSize: '0.8rem', color: 'var(--text-muted)' }}>Desk #{booking.desk.deskNumber}</div>
                  </td>
                  ...
                  <td style={{ padding: '1rem' }}>
                    <span
                      style={{
                        ...
                        fontSize: '0.75rem',
                        fontWeight: 600,
                        backgroundColor: getStatusStyle(booking.bookingStatus).bg,
                        color: getStatusStyle(booking.bookingStatus).color,
                      }}
                    >
